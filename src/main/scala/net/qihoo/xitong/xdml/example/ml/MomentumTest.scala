...
import net.qihoo.xitong.xdml.ml.LogisticRegressionWithMomentum
...
object MomentumTest {
	def main(args: Array[String]): Unit = {
		...
		val momCoff = jobConf.getOrElse(JobConfiguration.MOMENTUM_COFF, "0.1").toFloat
		...
		val tableName = System.getProperty("user.name") + "_" + jobConf.getOrElse(JobConfiguration.TABLE_NAME, "Momentum")
		...
		val model = new LogisticRegressionWithMomentum(ps)
			.setIterNum(iterNum)
			.setBatchSize(batchSize)
			.setLearningRate(learningRate)
			.setMomemtumCoff(momCoff)
