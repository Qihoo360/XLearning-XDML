...
import net.qihoo.xitong.xdml.ml.LogisticRegression
...
object LRTest {
	def main(args: Array[String]): Unit = {
		...
		val tableName = System.getProperty("user.name") + "_" + jobConf.getOrElse(JobConfiguration.TABLE_NAME, "LR")
		...
		val psConf = new PSConfiguration()
			.setPsDataType(PSDataType.FLOAT)
			.setPsDataLength(1)
		...
		if (jobType.toUpperCase.equals("TRAIN")) {
			...
			if (modelPath.equals("") || resultPath.equals(""))
				throw new XDMLException("Predict job must have a model path and result path!")
			else
		...
		val model = new LogisticRegression(ps)
			.setIterNum(iterNum)
		...
		psConf.getJobType match {
			case JobType.TRAIN => {
				...
				println("Save the model to path :" + modelPath)
