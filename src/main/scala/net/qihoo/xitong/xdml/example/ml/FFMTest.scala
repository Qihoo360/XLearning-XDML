...
import net.qihoo.xitong.xdml.dataProcess.FFMProcessor
import net.qihoo.xitong.xdml.ml.FieldawareFactorizationMachine
...
object FFMTest {
	def main(args: Array[String]): Unit = {
		...
		val rank = jobConf.getOrElse(JobConfiguration.FFM_RANK, "1").toInt
		val field = jobConf.getOrElse(JobConfiguration.FFM_FIELD, "1").toInt
		val tableName = System.getProperty("user.name") + "_" + jobConf.getOrElse(JobConfiguration.TABLE_NAME, "FFM")
		...
		val data = FFMProcessor.processData(rawData,split).coalesce(dataPartitionNum)
		...
		val model = new FieldawareFactorizationMachine(ps)
			.setIterNum(iterNum)
