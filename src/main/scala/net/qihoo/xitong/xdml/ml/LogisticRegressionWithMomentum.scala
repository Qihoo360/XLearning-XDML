...
import net.qihoo.xitong.xdml.updater.MomLRUpdater
import net.qihoo.xitong.xdml.optimization.BinaryClassify
import org.apache.spark.rdd.RDD

import scala.collection.JavaConversions._

class LogisticRegressionWithMomentum(ps:PS) extends Serializable {
	private var iterNum = 1
	private var batchSize = 50
	private var learningRate:Float = 0.01F
	private var momentumCoff:Float = 0.1F
	def fit(data: RDD[(Double, Array[Long], Array[Float])]): (Map[Int, Double], (Long, Long)) = {
		LogisticRegressionWithMomentum.runMomentum(ps,data,iterNum,batchSize,learningRate,momentumCoff)
	}
	...
	def setMomemtumCoff(coff:Float):this.type = {
		this.momentumCoff = coff
		this
	}
}

object LogisticRegressionWithMomentum{
	def runMomentum(ps:PS,
		...
		for (iterNum <- 0 until numIterations) {
			val result = data.mapPartitions { iter =>
				...
				val update = new MomLRUpdater()
					.setLearningRate(learningRate)
					.setCoff(coff)
				client.setUpdater(update)
