...
class WeightedLogitLossFunc(posWeight: Double) extends LossFunc {
  ...
  override def loss(data: Vector,
    ...
    if (label > 0.5) {
      ...
      MLUtils.log1pExp(margin) * posWeight
    } else {
  ...
  override def gradientWithLoss(data: Vector,
    ...
    (factor * (label * (posWeight - 1) + 1), loss)
  }

  override def gradientFromDot(dot: Double,
                               label: Double): Double = {
    (sigmoid(dot) - label) * (label * (posWeight - 1) + 1)
  }
