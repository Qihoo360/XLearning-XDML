...
trait MultiLinearScopeParams extends Params
...
class MultiLinearScope(override val uid: String) extends Estimator[MultiLinearScopeModel]
  with MultiLinearScopeParams with DefaultParamsWritable {

  def this() = this(Identifiable.randomUID("MultiLinearScope"))
  ...
  protected def getLossFunction(lossType: String, numClasses: Int): MultiLossFunc = {
    val lossFunc = lossType match {
      case "SR" =>
        new MultiLogitLossFunc(numClasses)
  ...
  protected def train(data: RDD[(Double, Vector)],
    ...
    if (data.isEmpty()) {
      ...
      return (initialWeightsArr, Array.fill(numClasses-1)(0.0))
    }
    ...
    val interceptArr = Array.fill(numClasses-1)(0.0)
    // learning loop
    var iter = 1
    while (iter <= numIterations) {
      ...
      if (fitIntercept) {
        ...
        val (gradientWeightsSumArr, gradientInterceptSumArr, lossSum, dataCount) = dataCached.treeAggregate(
          (Array.fill(numClasses-1)(Vectors.zeros(numWeights)), Array.fill(numClasses-1)(0.0), 0.0, 0L)
        )(
          seqOp = (c, v) => {
        ...
        val (weightsArrUpdated, interceptArrUpdated) = dataCached.mapPartitions(iter => {
          ...
          for (j <- 1 to indexedSeqLength) {
            ...
            val margins1 = Array.fill(numClasses-1)(0.0)
            val margins2 = Array.fill(numClasses-1)(0.0)
        ...
        val (gradientWeightsSumArr, lossSum, dataCount) = dataCached.treeAggregate(
          (Array.fill(numClasses-1)(Vectors.zeros(numWeights)), 0.0, 0L)
        )(
          seqOp = (c, v) => {
  ...
  override def fit(dataset: Dataset[_]): MultiLinearScopeModel = {
    ...
    // initialWeightsArr
    val initialWeightsArr = Array.fill(${numClasses}-1)(Vectors.zeros(numFeatures))
    // restartFrequency
    ...
    // model
    val model = new MultiLinearScopeModel(uid, weightsArr, interceptArr)
    copyValues(model)
  }

  // TODO: add initial intercept
  def fit(dataset: Dataset[_], initWeightsArr: Array[newVector]): MultiLinearScopeModel = {
    transformSchema(dataset.schema)
    // training data
    val trainData: RDD[(Double, Vector)] = dataset.select(
      col($(labelCol)).cast(DoubleType), col($(featuresCol))).rdd.map { case Row(label: Double, features: newVector) =>
  ...
  override def copy(extra: ParamMap): MultiLinearScope = defaultCopy(extra)

}

object MultiLinearScope extends DefaultParamsReadable[MultiLinearScope] {
  override def load(path: String): MultiLinearScope = super.load(path)
}

class MultiLinearScopeModel(override val uid: String, val coefficientsArr: Array[Vector], val interceptArr: Array[Double])
  extends Model[MultiLinearScopeModel] with MultiLinearScopeParams with HasRawPredictionCol with HasProbabilityCol with HasPredictionCol with MLWritable {

  import MultiLinearScopeModel._
  ...
  protected def predict(features: Vector): newVector = {
    val probs = Array.fill(${numClasses})(1.0)
    var sum = 1.0
    for(ind <- 0 until (${numClasses}-1)) {
      probs(ind+1) = math.exp(BLAS.dot(coefficientsArr(ind), features) + interceptArr(ind))
      sum += probs(ind+1)
    }
    newVectors.dense(probs.map(x => x/sum))
  }
  ...
  override def copy(extra: ParamMap): MultiLinearScopeModel = {
    val copied = new MultiLinearScopeModel(uid, coefficientsArr, interceptArr)
    copyValues(copied, extra).setParent(parent)
  }

  override def write: MLWriter = new MultiLinearScopeModelWriter(this)

}

object MultiLinearScopeModel extends MLReadable[MultiLinearScopeModel] {

  private[MultiLinearScopeModel]
  class MultiLinearScopeModelWriter(instance: MultiLinearScopeModel) extends MLWriter {
  ...
  private[MultiLinearScopeModel]
  class MultiLinearScopeModelReader extends MLReader[MultiLinearScopeModel] {

    private val className = classOf[MultiLinearScopeModel].getName

    override def load(path: String): MultiLinearScopeModel = {
      ...
      val model = new MultiLinearScopeModel(metadata.uid, coefficientsSeq.toArray, interceptSeq.toArray)
      DefaultParamsReader.getAndSetParams(model, metadata)
      model
    }
  }

  override def read: MLReader[MultiLinearScopeModel] = new MultiLinearScopeModelReader

  override def load(path: String): MultiLinearScopeModel = super.load(path)
